...
const Lunch = ({ foodDataLunchList }) => {
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 6;
  const startIndex = currentPage * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = foodDataLunchList.slice(startIndex, endIndex);
  ...
  const handleNext = () => {
    if (endIndex < foodDataLunchList.length) {
      setCurrentPage(currentPage + 1);
    }
  };
  return (
      ...
      <div className="flex flex-col">
        ...
        <div className="flex justify-center gap-4 my-10">
          ...
          <button
            onClick={handleNext}
            className="bg-[#f54748] rounded-[8px] font-medium text-[16px] text-white h-[45px] px-6"
            disabled={endIndex >= foodDataLunchList.length}
          >
            Next
          </button>
        </div>

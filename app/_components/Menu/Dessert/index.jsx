...
const Dessert = ({ foodDataDessertList }) => { 
  // console.log(foodDataDessertList)
  const [currentPage, setCurrentPage] = useState(0);
  const itemsPerPage = 6;
  const startIndex = currentPage * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = foodDataDessertList.slice(startIndex, endIndex);
      ...
      <div className="flex flex-col">
        ...
        <div className="flex justify-center gap-4 my-10">
          ...
          <button
            onClick={handleNext}
            className="bg-[#f54748] rounded-[8px] font-medium text-[16px] text-white h-[45px] px-6"
            disabled={endIndex >= foodDataDessertList.length}
          >
            Next
          </button>
        </div>
